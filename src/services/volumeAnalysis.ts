...
const parseObj = (text: string): Point3[] =>
  text.split(/\r?\n/).flatMap((line) => {
...
const parseStl = async (file: File): Promise<Point3[]> => {
  ...
  if (textPrefix.startsWith('solid')) {
    return [...new TextDecoder().decode(bytes).matchAll(/vertex\s+([^\s]+)\s+([^\s]+)\s+([^\s]+)/gi)].flatMap(
      (match) => {
...
const countAtOffset = (points: Point3[], size: number, offset: Point3) =>
  new Set(
    points.map(([x, y, z]) => {
...
const boxCounts3d = (points: Point3[]) =>
  [1, 2, 4, 8, 16, 32, 64].map((size) => {
    ...
    const count =
      offsets.reduce((sum, offset) => sum + countAtOffset(points, size, offset), 0) /
      offsets.length;
...
export async function analyzeMeshVolume(file: File): Promise<VolumeBoxCountResult> {
  ...
  const points = sample(
    extension === 'obj' ? parseObj(await file.text()) : await parseStl(file)
  );
  if (points.length < 8)
    throw new Error('The mesh has too few readable vertices. Upload a valid OBJ or STL geometry file.');
  const normalized = normalizePoints(points);
  const boxCounts = boxCounts3d(normalized.points);
  return {
    runId: `volume_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`,
    sourceKind: 'mesh',
    fractalDimension: dimension(boxCounts),
    boxCounts,
    pointCount: points.length,
    bounds: { x: normalized.extent[0], y: normalized.extent[1], z: normalized.extent[2] },
    elapsedSeconds: Number(((performance.now() - start) / 1000).toFixed(4)),
    gridOptimization: {
      enabled: true,
      offsetsPerScale: 5,
      method: 'five-phase translated-voxel-grid average',
    },
  };
}
...
const readDicomSlice = async (file: File): Promise<DicomSlice> => {
  ...
  let offset =
    ...
    new TextDecoder().decode(new Uint8Array(buffer, 128, 4)) === 'DICM'
      ? 132
      : 0;
  let width = 0,
    height = 0,
  ...
  while (offset + 8 <= view.byteLength) {
    const group = view.getUint16(offset, true),
      element = view.getUint16(offset + 2, true);
  ...
  if (!width || !height || pixelOffset < 0 || ![8, 16].includes(bits))
    throw new Error('Only uncompressed explicit-little-endian DICOM slices with 8- or 16-bit pixels are supported.');
  ...
  for (let i = 0; i < pixelCount; i += 1)
    pixels[i] = bits === 8 ? view.getUint8(pixelOffset + i) : view.getUint16(pixelOffset + i * 2, true);
...
export async function analyzeDicomStack(files: File[]): Promise<VolumeBoxCountResult> {
  ...
  if (slices.some((slice) => slice.width !== width || slice.height !== height))
    throw new Error('All DICOM slices must share the same pixel dimensions.');
  ...
  return {
    ...
    sourceKind: 'dicom-stack',
    fractalDimension: dimension(boxCounts),
    boxCounts,
    pointCount: points.length,
    bounds: { x: width, y: height, z: slices.length },
    elapsedSeconds: Number(((performance.now() - start) / 1000).toFixed(4)),
    gridOptimization: {
      enabled: true,
      offsetsPerScale: 5,
